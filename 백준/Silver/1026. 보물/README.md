...
1. Python에서 리스트의 최댓값,최솟값을 반환 하는 함수 max(리스트 이름),min(리스트 이름) 에 대한 지식이 부족
2. 리스트.sort() 와 sorted(리스트) 의 차이점 - 또한 sorted(리스트, reverse=True) 진행 시 리스트 값 내림차순 배열 , sorted(리스트, reverse=False) 진행 시 리스트 값 오름차순 배열
